...
def transform_model_params(
  ...
  model_params_nocut = split_flow_nocut(
      concat_params=concat_params_nocut, num_groups=num_groups)
  ...
  model_params_cut = split_flow_cut(
      concat_params=concat_params_cut, num_groups=num_groups)
...
def sample_and_evaluate(config: ConfigDict, workdir: str) -> Mapping[str, Any]:
  ...
  if os.path.exists(samples_path):
    ...
    if os.path.exists(samples_path_stg1):
      ...
      model_params_stg1_unb_init = jax.vmap(lambda x: init_param_fn_stg1(
          prng_key=x,
      ...
      initial_states_stg1, hmc_params_stg1 = jax.vmap(lambda x, y: call_warmup(
          prng_key=x,
  ...
  plot.posterior_plots(
      az_data=az_data,
      show_sigma_trace=True,
      show_beta_trace=True,
      show_tau_trace=True,
      betasigma_pairplot_groups=(1, 2, 3),
      tausigma_pairplot_groups=(1, 2, 3),
